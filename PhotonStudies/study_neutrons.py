...
#these have to be able to handle lots of hits per event
Hhit_x = array('d', [0]*100)
Hhit_y = array('d', [0]*100)
Hhit_z = array('d', [0]*100)
Hhit_phi =array('d', [0]*100)
Hhit_eta = array('d', [0]*100)
Hhit_E = array('d', [0]*100)
Hhit_depth = array('d', [0]*100)
Hhit_time = array('d', [0]*100)
...
#hit info
neutron_tree.Branch("Hhit_x", Hhit_x, 'var/D')
neutron_tree.Branch("Hhit_y", Hhit_y, 'var/D')
neutron_tree.Branch("Hhit_z", Hhit_z, 'var/D')
neutron_tree.Branch("Hhit_phi", Hhit_phi, 'var/D')
neutron_tree.Branch("Hhit_eta", Hhit_eta, 'var/D')
neutron_tree.Branch("Hhit_E", Hhit_E, 'var/D')
neutron_tree.Branch("Hhit_time", Hhit_time, 'var/D')
neutron_tree.Branch("Hhit_depth", Hhit_depth, 'var/D')


#neutron_tree.Branch("N_photon", N_photon, 'var/D')
...
for file in to_process:
    ...
    for ievt, event in enumerate(reader):
        ...
        #vtx3 = mcpCollection[0].getVertex()
        #vtx3_pos = vtx3.getPosition()
        ...
        pT_truth[0] = tlv.Perp()
        pdg_truth[0] = 2112
        truth_ProdVtx_x[0] = 0.0 #vtx3_pos[0]
        truth_ProdVtx_y[0] = 0.0 #vtx3_pos[1]
        truth_ProdVtx_z[0] = 0.0 #vtx3_pos[2]
        ...
        for pfo in pfoCollection:
            ...
            '''
            pT_pfo = tlv_pfo.Perp()
            pdg_id[0] = abs(pfo.getType())
            pfo_vtx3 = pfo.getStartVertex()
            #pfo_vtx3_pos = pfo_vtx3.getPosition()
            ProdVtx_x[0] = 0.0 #pfo_vtx3_pos[0]
            ProdVtx_y[0] = 0.0 #pfo_vtx3_pos[1]
            ProdVtx_y[0] = 0.0 #pfo_vtx3_pos[2]
            #pfo_dvtx3 = pfo.getEndVertex()
            #pfo_dvtx3_pos = pfo.getPosition()
            #DecayVtx_x[0] = pfo_dvtx3_pos[0]
            #DecayVtx_y[0] = pfo_dvtx3_pos[1]
            #DecayVtx_z[0] = pfo_dvtx3_pos[2]
            ...
            if pfo.getEnergy() > 50. and dR < 0.05:
                if pT_pfo > maxpT and abs(pfo.getType()) == 2112:
                    hasNeutron = True
                    maxpT = pT_pfo
                    #minDREM = dR
                ...
                if abs(pfo.getType()) == 2112 or abs(pfo.getType()) == 22:
                    E[0] += pfo.getEnergy()
       ...
       ''' 
        ...
        for icoll, coll in enumerate(hcal_coll):
            ...
            try:
                ...
                HCAL_sumhits += len(ECALhitCollection)
                hit_iter=0
                for hit in HCALhitCollection:
                    cellID = int(hit.getCellID0())
                    decoder.setValue(cellID)
                    layer = decoder["layer"].value()
                    Hhit_time.append(hit.getTime())
                    ...
                    pos = hit.getPosition()
                    Hhit_x[hit_iter] = pos[0]
                    Hhit_y[hit_iter] = pos[1]
                    Hhit_z[hit_iter] = pos[2]

                    h_HCAL_hit_R.Fill(sqrt(pos[0]*pos[0]+pos[1]*pos[1]))

                    E_cell[hit_iter] = hit.getEnergy()
                    Hhit_E.append(hit.getEnergy())
                    hit_vec = TLorentzVector(pos[0], pos[1], pos[2], hit.getTime())
                    
                    Hhit_eta[hit_iter] = hit_vec.Eta()
                    Hhit_phi[hit_iter] = hit_vec.Phi()
                    Hhit_depth[hit_iter] = layer
