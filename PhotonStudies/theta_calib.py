...
for tbin in range(0, len(ThetaBins)-1):
    ...
    for Ebin in range(0, len(EBins)-1):
        ...
        if len(ratios) > 0:
            ...
            #plt.close()
            #UNCOMMENT BELOW to start removing outliers
            #for ratio in ratios:
                #if ratio/np.average(ratios) > 50.0:
                    #ratios.remove(ratio)
            #ratios.remove(max(ratios))
            #print(max(ratios)/np.average(ratios))
            ...
            for tbin in range(0, len(ThetaBins)-1):
                ...
                if EMin <= E_truth and E_truth < EMax:
                    ...
                    if ThMin <= theta and theta < ThMax:
                        ...
                        if (E_corr - E_truth)/E_truth > -10000.5 and (E_corr - E_truth)/E_truth < 10000.5:
